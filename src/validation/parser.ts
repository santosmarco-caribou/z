...
export const DEFAULT_PARSE_OPTIONS: Joi.ValidationOptions & Required<ParseOptions> = {
  abortEarly: false,
}
...
export class ZParser<Def extends AnyZDef> {
  ...
  private _validate(input: unknown, options: ParseOptions | undefined): Joi.ValidationResult<ZOutput<this>> {
    const _opts = mergeSafe(DEFAULT_PARSE_OPTIONS, options ?? {})
