...
export type ZIssueMsgLabelKey<Key extends string = string> =
  Key extends keyof RemoveIndexSignature<Joi.Context> ? never : Key

type _ZIssueMsgLabelTypeMap = {
  string: string
  number: number
  boolean: boolean
}

export type ZIssueMsgLabelType = keyof _ZIssueMsgLabelTypeMap

export type ZIssueMsgLabel<
  Key extends string = string,
  Type extends ZIssueMsgLabelType = ZIssueMsgLabelType
> = `{{#${ZIssueMsgLabelKey<Key>}::${Type}}}`

export const createZIssueMsgLabel = <
  Key extends string,
  Type extends ZIssueMsgLabelType
>(
  key: ZIssueMsgLabelKey<Key>,
  type: Type
): ZIssueMsgLabel<Key, Type> => `{{#${key}::${type}}}`

/* --------------------------------- Message -------------------------------- */

const _ZIssueDefaultMessagePrefix = `{{#label::string}}`

export type ZIssueDefaultMessagePrefix = typeof _ZIssueDefaultMessagePrefix

export type ZIssueMessage<Msg extends string = string> =
  `${ZIssueDefaultMessagePrefix} ${Msg extends `${string}.` ? Msg : `${Msg}.`}`

export const createZIssueMsg = <Msg extends string>(
  message: Msg
): ZIssueMessage<Msg> =>
  `${_ZIssueDefaultMessagePrefix} ${message}` as ZIssueMessage<Msg>

/* ----------------------------------- Map ---------------------------------- */

export type ZIssueMap<Type extends ZType> = {
  [K in `${ZType}`]: Simplify<
    Record<`${K}.base`, ZIssueMessage> &
      Record<`${K}.${string}`, ZIssueMessage>,
    { deep: true }
  >
}[Type]
...
>(
  ...
  Object.fromEntries(
    Object.entries<string>(map as Map).map(([k, v]) => [
      `${typeName}.${k}`,
      `${_ZIssueDefaultMessagePrefix} ${v}`,
    ])
  ) as _AddMapPrefixes<TypeName, Map>

/* ---------------------------------- Code ---------------------------------- */

export type ZIssueCode<Type extends ZType> = keyof ZIssueMap<Type>

export type AnyZIssueCode = ZIssueCode<ZType>

/* --------------------------------- Context -------------------------------- */

type _ZIssueContextRaw<Msg extends ZIssueMessage> = Extract<
  Msg,
  `{{#${string}}}`
> extends `{{#${infer LabelKey}::${infer Type}}}`
  ? {
      [K in Extract<
        LabelKey,
        ZIssueMsgLabelKey<LabelKey>
      >]: Type extends ZIssueMsgLabelType ? _ZIssueMsgLabelTypeMap[Type] : any
    }
  : EmptyObject

export type ZIssueContext<
  Msg,
  Opts extends { Extras: boolean } = { Extras: false }
> = Msg extends ZIssueMessage
  ? Opts['Extras'] extends true
    ? Merge<SetRequired<Joi.Context, 'value'>, _ZIssueContextRaw<Msg>>
    : _ZIssueContextRaw<Msg>
  : never
