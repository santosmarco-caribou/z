...
export type CustomValidationFail =
  ...
  | ZError
  | [
      typeof CUSTOM_VALIDATION_FAIL,
      AnyZIssueCode,
      Omit<ZIssueLocalContext<AnyZIssueCode>, 'label'>
    ]
export type CustomValidationResult<Out> =
  | CustomValidationOk<Out>
  | CustomValidationFail
...
export class ZCustom<Output, Input = unknown> extends Z<{
  ...
  static create = <Output, Input = unknown>(
    validate: (
      value: Input,
      helpers: CustomValidationHelpers<Output>
    ) => CustomValidationResult<Output>
    ...
    const schema = ZJoi.any().custom((value, _helpers) => {
